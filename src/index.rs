use {
  self::{
    entry::{
      ...
      OutPointValue, DuneEntryValue, DuneIdValue, SatPointValue, SatRange,
    },
    dunes::{Dune, DuneId},
    updater::Updater,
  },
...
impl Index {
    ...
    let database = match unsafe { Database::builder().open(&path) } {
      ...
      Err(DatabaseError::Storage(StorageError::Io(error))) if error.kind() == io::ErrorKind::NotFound => {
...

        {
          ...
          statistics.insert(
            &Statistic::IndexDunes.key(),
            &u64::from(index_dunes),
          )?;
  ...
  pub(crate) fn info(&self) -> Result<Info> {
    ...
    let info = {
      ...
      let transactions: Vec<TransactionInfo> = wtx
        ...
        .range(0..)?
        .map(|result| result.map(|(starting_block_count, starting_timestamp)| TransactionInfo {
  ...
  pub(crate) fn get_dune_by_inscription_id(
    ...
    let Some(dune) = rtx
        ...
        .map(|entry| Dune(entry.value())) else {
      return Ok(None);
    };
    let dune_to_dune_id = rtx.open_table(DUNE_TO_DUNE_ID)?;
  ...
  pub(crate) fn get_latest_inscriptions_with_prev_and_next(
    ...
    let inscriptions = inscription_number_to_inscription_id
      ...
      .take(n)
      .map(|result| {
        result.map(|(_number, id)| Entry::load(*id.value()))
          .map_err(|e| e.into())
      })
